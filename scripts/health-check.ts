...
class HealthChecker {
  ...
  private addResult(component: string, healthy: boolean, message: string, details?: Record<string, unknown>): void {
  ...
  async checkNodeVersion(): Promise<void> {
    ...
    this.addResult(
      'Node.js Version',
      healthy,
      healthy ? `Node.js ${nodeVersion} ✓` : `Node.js ${nodeVersion} - requires 18.0.0 or higher`,
      { version: nodeVersion, majorVersion }
    );
  }
  ...
  async checkDirectories(): Promise<void> {
    ...
    this.addResult(
      ...
      allHealthy 
        ...
        : `Some directories missing - check configuration`,
      { directories: directoryStatus }
    );
  }

  async checkLogDirectory(): Promise<void> {
    ...
    try {
      ...
      this.addResult(
        'Log Directory',
        false,
        `Cannot write to log directory: ${logDir}`,
        { logDirectory: logDir, error: (error as Error).message }
      );
    }
  }

  async checkModelCache(): Promise<void> {
    ...
    try {
      ...
      this.addResult(
        ...
        true,
        `Model cache directory ready: ${cacheDir} ✓`,
      ...
      this.addResult(
        'Model Cache Directory',
        false,
        `Cannot create model cache directory: ${cacheDir}`,
        { cacheDirectory: cacheDir, error: (error as Error).message }
      );
    }
  }

  async checkQdrant(): Promise<void> {
    try {
      ...
      if (healthy) {
        ...
        this.addResult(
          'Qdrant Database',
          true,
          `Qdrant is healthy at ${config.qdrant.url} ✓`,
          { url: config.qdrant.url, status: healthData }
        );
      } else {
        this.addResult(
          'Qdrant Database',
          false,
          `Qdrant health check failed: HTTP ${response.status}`,
          { url: config.qdrant.url, status: response.status }
        );
      }
    } catch (error) {
      this.addResult(
        'Qdrant Database',
        false,
        `Cannot connect to Qdrant at ${config.qdrant.url}. Is it running?`,
        { 
  ...
  async checkDependencies(): Promise<void> {
    ...
    this.addResult(
      ...
      allInstalled 
        ...
        : `Some dependencies missing - run: pnpm install`,
  ...
  async checkConfiguration(): Promise<void> {
    ...
    this.addResult(
      'Configuration',
      healthy,
      healthy ? 'Configuration is valid ✓' : `Configuration issues: ${issues.join(', ')}`,
      { 
        issues,
